...
12. `service` 中涉及新增、修改的业务时，如果字段具有唯一性要求，必须先通过 `manager` 查询数据库，校验是否已存在重复数据。
13. 重复校验只针对有唯一性语义的字段，例如角色名称、角色编码、用户名、手机号、邮箱等，不做无意义的全字段判重。
14. 修改场景做唯一字段校验时，必须排除当前正在修改的数据本身，避免把自己判重。
15. 如果业务规则明确允许重复，则不要机械增加唯一性校验，应按实际业务规则实现。
